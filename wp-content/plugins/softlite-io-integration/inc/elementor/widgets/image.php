...
class Softlite_Image extends \Elementor\Widget_Base {
	...
	protected function render() {
		...
		if ( empty( $settings['image']['url'] ) && 'upload' === $settings['image_source'] ) {
			return;
		}
		...
		<?php } ?>
			...
			<?php endif; ?>
				<?php if (empty($settings['image_source']) || 'external' !== $settings['image_source']) : ?>
					<?php Elementor\Group_Control_Image_Size::print_attachment_image_html( $settings ); ?>
				<?php else : ?>
					<img src="<?php echo esc_url( $settings['image_external_url'] ); ?>">
				<?php endif; ?>
			<?php if ( $link ) : ?>
					</a>
			<?php endif; ?>
	...
	protected function content_template() {
		?>
		<# 
			if (settings.image_source === 'upload') {
				var image = {
				...
				var image = {
					id: '',
					url: settings.image_external_url,
					size: settings.image_size,
					dimension: settings.image_custom_dimension,
					model: view.getEditModel()
				};
